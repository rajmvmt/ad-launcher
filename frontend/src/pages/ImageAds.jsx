...
import { ChevronRight, ChevronLeft, Check, Briefcase, Package, Users, Image, Hash, FileText, Sparkles, Download, ChevronDown, ChevronUp, Settings, CheckCircle2, ArrowRight, FlaskConical, Zap, Crown, Loader2, Wand2, Upload, X, ImagePlus, MessageCircle, Heart } from 'lucide-react';
    ...
    const handleGenerate = async () => {
        ...
        try {
            const payload = {
                ...
                model: aiModel,
                language,
                // Akemi Before & Afters always generates exactly 1 variation
                variationCount: aiModel === 'akemi_before_after' ? 1 : wizardData.variationCount,
                research_doc_id: selectedResearchDocId || null,
                referenceImageUrls: (wizardData.referenceImages || []).map(img => img.url),
            };
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 min-h-[500px] relative">
                ...
                {currentStep === 7 && (
                    <CampaignDetailsStep
                        ...
                        referenceImages={wizardData.referenceImages || []}
                        onReferenceImagesChange={(imgs) => updateData('referenceImages', imgs)}
                        authFetch={authFetch}
                        selectedResearchDocId={selectedResearchDocId}
                    />
                )}
                ...
                {currentStep === 8 && (
                    <ReviewStep
                        ...
                        selectedResearchDocId={selectedResearchDocId}
                        setSelectedResearchDocId={setSelectedResearchDocId}
                        authFetch={authFetch}
                    />
                )}
                ...
                <div className="mt-6 flex items-center justify-between">
                    ...
                    {currentStep === 8 ? (
                        ...
                        >
                            ...
                            {generating ? `Generating with ${aiModel === 'sonnet' ? 'Sonnet' : aiModel === 'haiku' ? 'Haiku' : aiModel === 'group_voice' ? 'Group Voice' : aiModel === 'akemi_before_after' ? 'Akemi B&A' : 'Gemini'}...` : 'Generate Ad Copy'}
                        </button>
                    ) : (
                        <button
                            onClick={nextStep}
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                >
                                    ...
                                    <p className="text-sm text-gray-600">Google's best image model via fal.ai. Great text rendering, photorealistic. $0.15/image. (Default)</p>
                                </div>
                                ...
                                >
                                    ...
                                    <p className="text-sm text-gray-600">Direct Imagen 4 via Gemini API. High fidelity, photorealistic. Free with API key.</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
function CampaignDetailsStep({ details, onChange, brand, product, profile, referenceImages, onReferenceImagesChange, authFetch, selectedResearchDocId }) {
    ...
    const handleAiGenerate = async () => {
        if (!brand || !product || !profile) return;
        setAiGenerating(true);
        try {
            const res = await authFetch(`${API_URL}/copy-generation/generate-campaign-details`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    brand,
            ...
            if (data.offer) onChange('offer', data.offer);
            if (data.urgency) onChange('urgency', data.urgency);
            if (data.messaging) onChange('messaging', data.messaging);
            if (data.angle) onChange('angle', data.angle);
        } catch (e) {
            console.error('AI campaign details error:', e);
        }
        ...
        try {
            for (const file of files) {
                ...
                if (res.ok) {
                    ...
                    if (url) {
                        onReferenceImagesChange([...referenceImages, { url, name: file.name }]);
                    }
                }
            }
        } catch (err) {
        ...
        <div>
            <div className="flex items-center justify-between mb-4">
                ...
                <button
                    ...
                    disabled={aiGenerating || !brand || !product || !profile}
                    className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 disabled:opacity-50 text-sm font-medium shadow-sm transition-all"
                    title="AI will generate all fields based on your brand, product & research docs"
                >
                    {aiGenerating ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={16} />}
                    {aiGenerating ? 'Generating...' : 'AI Fill All Fields'}
                </button>
            </div>
            ...
            <div className="max-w-2xl space-y-4">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        placeholder="e.g., Senior man sitting on bed at 3AM holding foot in pain, bedroom lamp on, expression of exhaustion. OR: Same man smiling putting on compression sock with easy zipper."
                        rows={3}
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                    />
                ...
                <div className="pt-2">
                    ...
                    <p className="text-xs text-gray-500 mb-3">
                        Upload product photos or reference images. These help AI write more accurate copy and generate better visuals.
                    </p>

                    <div className="flex flex-wrap gap-3 mb-3">
                        {referenceImages.map((img, idx) => (
                            <div key={idx} className="relative group w-20 h-20 rounded-lg border border-gray-200 overflow-hidden bg-gray-50">
                        ...
                        <button
                            ...
                            disabled={uploading}
                            className="w-20 h-20 rounded-lg border-2 border-dashed border-gray-300 hover:border-amber-400 flex flex-col items-center justify-center text-gray-400 hover:text-amber-600 transition-colors"
                        >
...
function ReviewStep({ wizardData, aiModel, setAiModel, language, setLanguage, selectedResearchDocId, setSelectedResearchDocId, authFetch }) {
    const [showPrompt, setShowPrompt] = useState(false);
    const [customPrompt, setCustomPrompt] = useState('');
    const [researchDocs, setResearchDocs] = useState([]);

    // Fetch brand's research docs
    React.useEffect(() => {
        if (!wizardData.brand?.id) return;
        const fetchDocs = async () => {
            try {
                const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000/api/v1';
                const res = await authFetch(`${API_URL}/prompts/?type=research&brand_id=${wizardData.brand.id}`);
                if (res.ok) {
                    const docs = await res.json();
                    setResearchDocs(docs);
                    if (docs.length === 1 && !selectedResearchDocId) setSelectedResearchDocId(docs[0].id);
                }
            } catch {}
        };
        fetchDocs();
            ...
            <div className="mt-6 max-w-3xl space-y-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 text-sm font-medium transition-all ${
                                aiModel === 'haiku' ? 'border-amber-500 bg-amber-50 text-amber-800' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                            }`}
                        >
                            <Zap size={16} /> Quick Copy (Haiku)
                        </button>
                        <button
                            onClick={() => setAiModel('sonnet')}
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 text-sm font-medium transition-all ${
                                aiModel === 'sonnet' ? 'border-purple-500 bg-purple-50 text-purple-800' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                            }`}
                        >
                            <Crown size={16} /> Premium Copy (Sonnet)
                        </button>
                        <button
                            onClick={() => setAiModel('gemini')}
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 text-sm font-medium transition-all ${
                                aiModel === 'gemini' ? 'border-blue-500 bg-blue-50 text-blue-800' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                            }`}
                        >
                            <Sparkles size={16} /> Gemini Flash
                        </button>
                        <button
                            onClick={() => setAiModel('group_voice')}
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 text-sm font-medium transition-all ${
                                aiModel === 'group_voice' ? 'border-green-500 bg-green-50 text-green-800' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                            }`}
                        >
                            <MessageCircle size={16} /> FB Group Voice
                        </button>
                        <button
                            onClick={() => setAiModel('akemi_before_after')}
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 text-sm font-medium transition-all ${
                                aiModel === 'akemi_before_after' ? 'border-pink-500 bg-pink-50 text-pink-800' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                            }`}
                        >
                            <Heart size={16} /> Akemi B&A
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select
                        ...
                        className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white"
                    >
                ...
                {researchDocs.length > 0 && (
                    <div>
                        ...
                        >
                            <option value="">None — generate without research context</option>
                            {researchDocs.map(doc => (
                                <option key={doc.id} value={doc.id}>{doc.name}</option>
                            ))}
                        </select>
                        {selectedResearchDocId && (
                            <p className="text-xs text-green-600 mt-1">Copy will be grounded in real product details from this research doc</p>
                        )}
                    </div>
                )}
            </div>
...
function CopySelectionStep({ generatedCopy, wizardData, aiModel, language, onBack, onRegenerate, isRegenerating, onProceed, customImagePrompt, setCustomImagePrompt, authFetch }) {
