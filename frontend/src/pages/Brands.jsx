...
import { Plus, Edit2, Trash2, Briefcase, LayoutGrid, List, FlaskConical } from 'lucide-react';
...
const Brands = () => {
    ...
    useEffect(() => {
        const fetchResearchCounts = async () => {
            try {
                const res = await authFetch(`${API_URL}/prompts/?type=research`);
                if (res.ok) {
                                ...
                                <div key={brand.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden group hover:shadow-md transition-shadow">
                                    ...
                                    <div className="pt-10 p-6">
                                        ...
                                        <div className="space-y-3 text-sm text-gray-600">
                                            ...
                                            {researchCounts[brand.id] > 0 && (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); navigate('/prompts?tab=research&brand=' + brand.id); }}
                                                    className="flex items-center gap-1.5 text-amber-600 hover:text-amber-700 hover:underline"
                                                >
                                                    <FlaskConical size={14} />
                                                    <span className="font-medium">{researchCounts[brand.id]} Research Doc{researchCounts[brand.id] > 1 ? 's' : ''}</span>
                                                </button>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Research</th>
