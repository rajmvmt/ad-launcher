...
import { ChevronRight, Plus, Check, Loader, X, Bookmark, Shuffle, RefreshCw } from 'lucide-react';
import { useCampaign } from '../context/CampaignContext';
import { useToast } from '../context/ToastContext';
import { getAdSets, getPixels, createPixel, searchGeoLocations, getAudiences } from '../lib/facebookApi';
import { createTemplate } from '../api/campaignTemplates';
...
const AdSetStep = ({ onNext, onBack }) => {
    const { campaignData, adsetData, setAdsetData, selectedAdAccount, selectedConnection } = useCampaign();
    ...
    const fetchPixels = async (forceRefresh = false) => {
        setLoadingPixels(true);
        try {
            const fetchedPixels = await getPixels(selectedAdAccount.id, selectedConnection?.id, forceRefresh);
            setPixels(fetchedPixels);
            if (forceRefresh) {
                showSuccess(`Pixel list refreshed (${fetchedPixels.length} pixels)`);
            }
        } catch (error) {
    ...
    const fetchAudiences = async () => {
        ...
        try {
            const fetched = await getAudiences(selectedAdAccount.id, selectedConnection?.id);
    ...
    const handleCreatePixel = async () => {
        ...
        try {
            const result = await createPixel(selectedAdAccount.id, newPixelName.trim());
            showSuccess(`Pixel "${newPixelName.trim()}" created`);
            setNewPixelName('');
            setShowCreatePixel(false);
            await fetchPixels();
            if (result.id) {
                handleInputChange('pixelId', result.id);
            }
        } catch (error) {
                            ...
                            {loadingPixels ? (
                                ...
                                <>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => fetchPixels(true)}
                                            disabled={loadingPixels}
                                            className="p-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-600 hover:text-amber-600 disabled:opacity-50"
                                            title="Refresh pixel list from Facebook (bypasses 24h cache)"
                                        >
                                            <RefreshCw size={18} className={loadingPixels ? 'animate-spin' : ''} />
                                        </button>
                                        <button
                                            type="button"
                                    ...
                                    {showCreatePixel && (
                                        <div className="mt-2 flex items-center gap-2">
                                            ...
                                            >
                                                {creatingPixel ? <Loader className="animate-spin" size={14} /> : 'Create'}
                                            </button>
                                            <button
                                                type="button"
                                ...
                                <div>
                                    ...
                                    {loadingAudiences ? (
                                        <div className="flex items-center gap-2 text-sm text-gray-500">
                                            <Loader size={14} className="animate-spin" /> Loading audiences...
                                        </div>
                                    ) : audiences.length === 0 ? (
                                        <p className="text-sm text-gray-400">No audiences found for this account</p>
                                    ) : (
                                        <>
                                            <div className="space-y-2">
                                                {/* Selected audiences */}
                                                {(adsetData.targeting?.custom_audiences || []).map(aid => {
                                                    const aud = audiences.find(a => a.id === aid);
                                                    return (
                                                        <span key={aid} className="inline-flex items-center gap-1 px-2.5 py-1 mr-2 mb-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                                                            {aud?.name || aid}
                                                            {aud?.subtype === 'LOOKALIKE' && <span className="text-blue-500">(LAL)</span>}
                                                            <button onClick={() => {
                                                                const updated = (adsetData.targeting?.custom_audiences || []).filter(id => id !== aid);
                                                                handleInputChange('targeting', { ...adsetData.targeting, custom_audiences: updated });
                                                            }} className="text-blue-500 hover:text-blue-700 ml-0.5"><X size={12} /></button>
                                                        </span>
                                                    );
                                                })}
                                            </div>
                                            <select
                                                onChange={(e) => {
                                                    ...
                                                    if (!val) return;
                                                    const current = adsetData.targeting?.custom_audiences || [];
                                                    if (!current.includes(val)) {
                                                        handleInputChange('targeting', { ...adsetData.targeting, custom_audiences: [...current, val] });
                                                    }
                                                    e.target.value = '';
                                                }}
                                                defaultValue=""
                                                className="mt-2 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm"
                                            >
                                                <option value="">+ Add an audience...</option>
                                                <optgroup label="Custom Audiences">
                                                    {audiences.filter(a => a.subtype !== 'LOOKALIKE').map(a => (
                                                        <option key={a.id} value={a.id} disabled={(adsetData.targeting?.custom_audiences || []).includes(a.id)}>
                                                            {a.name} {a.approximate_count ? `(~${(a.approximate_count / 1000).toFixed(0)}k)` : ''}
                                                        </option>
                                                    ))}
                                                </optgroup>
                                                <optgroup label="Lookalike Audiences">
                                                    {audiences.filter(a => a.subtype === 'LOOKALIKE').map(a => (
                                                        <option key={a.id} value={a.id} disabled={(adsetData.targeting?.custom_audiences || []).includes(a.id)}>
                                                            {a.name} {a.approximate_count ? `(~${(a.approximate_count / 1000).toFixed(0)}k)` : ''}
                                                        </option>
                ...
                <div className="flex items-center gap-3">
                    ...
                    {mode === 'new' && (
                        <div className="relative">
                            ...
                            {saveTemplateOpen && (
                                <div className="absolute bottom-full mb-2 left-0 bg-white border border-gray-200 rounded-lg shadow-xl p-3 w-72 z-50">
                                    ...
                                    <input
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter' && templateName.trim()) {
                                                ...
                                                const cc = { objective: campaignData.objective, budgetType: campaignData.budgetType, dailyBudget: campaignData.dailyBudget, bidStrategy: campaignData.bidStrategy, bidAmount: campaignData.bidAmount };
                                                const ac = { optimizationGoal: adsetData.optimizationGoal, dailyBudget: adsetData.dailyBudget, bidStrategy: adsetData.bidStrategy, bidAmount: adsetData.bidAmount, targeting: adsetData.targeting, advantageAudience: adsetData.advantageAudience, pixelId: adsetData.pixelId, conversionEvent: adsetData.conversionEvent, attributionSetting: adsetData.attributionSetting };
                                                createTemplate(templateName.trim(), cc, ac).then(() => { showSuccess('Template saved!'); setSaveTemplateOpen(false); setTemplateName(''); }).catch(e => showError(e.message)).finally(() => setSavingTemplate(false));
                                            }
                                        }}
                                        placeholder="e.g. US Purchase CBO $50/day"
                                        className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:ring-2 focus:ring-amber-500"
                                        autoFocus
                                    />
                                    <div className="flex gap-2 justify-end">
                                        <button onClick={() => setSaveTemplateOpen(false)} className="px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
                                        <button
                                            onClick={() => {
