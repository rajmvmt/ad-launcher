...
When working on mobile, ALWAYS push, commit, create a PR, and merge without asking. Do not prompt for confirmation — complete the full cycle automatically: commit → push → create PR → merge.
...
Ad Launcher - A full-stack application for automating the lifecycle of Facebook video and image ads, from competitor research to ad creation, launching, and performance reporting.

**Tech Stack:**
- Frontend: React 19 + Vite + TailwindCSS
- Backend: Python FastAPI (Python 3.11+)
- Database: Railway PostgreSQL (production) / local PostgreSQL 17 (dev)
- Storage: Cloudflare R2 (S3-compatible)
- Testing: agent-browser (e2e), Vitest (unit)
- Hosting: **Railway** (backend + frontend services). Local dev: `uvicorn` + `npm run dev`.
...
Local Postgres 17 only (Railway cancelled 2026-05-06). Connect with `postgresql://roly:localdev@localhost:5432/mvmt_printer`.
...
- Local Postgres only (Railway gone). R2 still used for image storage. Crons run via user `crontab` — laptop must be on.
