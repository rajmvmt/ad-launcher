...
def read_brands(
    ...
    brands = db.query(BrandModel).options(
        joinedload(BrandModel.products),
        joinedload(BrandModel.profiles)
...
def update_brand(
    ...
    db_brand.style_guide = brand.style_guide
    ...
    if brand.profileIds is not None:
        ...
        db_brand.profiles = []
        # Add new associations (batch fetch)
        if brand.profileIds:
            profiles = db.query(ProfileModel).filter(ProfileModel.id.in_(brand.profileIds)).all()
            for profile in profiles:
                db_brand.profiles.append(profile)
